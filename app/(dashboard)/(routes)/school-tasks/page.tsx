...
import { SchoolTasks_Prompts } from "@/constants";
...
const SchoolTasks = () => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      const response = await axios.post("/api/schooltasks", formData, {
        headers: {
  ...
  return (
    <div className="h-[calc(100vh-164px)] overflow-hidden">
      <Heading
        title={"School Tasks"}
        description="Your School homework tasks AI Assistant"
        icon={School}
                  ...
                  <FormItem className="col-span-10 lg:col-span-11">
                    <FormControl className="m-0 p-0">
                      ...
                      {/* <Textarea
                        ...
                        disabled={isLoading}
                        placeholder="If a triangle has sides of 5 cm, 4 cm, and 6 cm, what is its perimeter?"
                        onKeyDown={(e) => {
                          const isMobile =
          ...
          {messages.length === 0 && !isLoading && (
            <div className="">
              <Empty
                ...
                suggestions={SchoolTasks_Prompts}
              />
            </div>
          )}
